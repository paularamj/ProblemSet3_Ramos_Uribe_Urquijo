...
### Decidimos realizar la imputaci髇 con la info del CENSO del DANE
...
########## Revisamos variable BA袿S #############

#================== Imputar NAs ba駉s con info manzanas del DANE ====================#
...
housing_chapinero = housing_chapinero %>%
  group_by(MANZ_CCNCT) %>%
  mutate(ba駉s_mediana=median(bathrooms,na.rm=T))

table(is.na(housing_chapinero$bathrooms)) ## Tenemos 2641 NAs

table(is.na(housing_chapinero$bathrooms),
      is.na(housing_chapinero$ba駉s_mediana)) # logramos recuperar 117
...
table(is.na(housing_poblado$bathrooms),
      is.na(housing_poblado$ba駉s_mediana)) # logramos recuperar 4

#Imputar Medianas Manzanas del # de ba駉s con info de las manzanas del DANE

# Chapinero


housing_chapinero = housing_chapinero %>% group_by(MANZ_CCNCT) %>% 
  mutate(new_ba駉s_vf = ifelse(is.na(bathrooms),
                                 yes = ba駉s_mediana,
                                 no = bathrooms))
...
table(is.na(housing_chapinero$new_ba駉s_vf))

#Poblado

table(is.na(housing_poblado$new_ba駉s_vf))

#Eliminamos NAs que no pudimos capturar

#Chapinero

housing_chapinero <- housing_chapinero[!is.na(housing_chapinero$new_ba駉s_vf),]

#Poblado

housing_poblado <- housing_poblado[!is.na(housing_poblado$new_ba駉s_vf),]
...
sum(is.na(housing_chapinero$new_ba駉s_vf))
...
sum(is.na(housing_poblado$new_ba駉s_vf))
